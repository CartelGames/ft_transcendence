...
def index(request):
    if request.method == 'POST':
        if request.POST.get('type') == 'login':
        ...
        elif request.POST.get('type') == 'blockFriend':
            if isinstance(request.user, AnonymousUser):
                return JsonResponse({'success': False, 'csrf_token': get_token(request)})
            ...
            return JsonResponse({'success': False, 'errors': 'An error occured with the type of post.', 'csrf_token': get_token(request)})
        ...
        elif request.GET.get('data') == 'blocked_friends':
            if isinstance(request.user, AnonymousUser):
                return JsonResponse({'success': False, 'csrf_token': get_token(request)})
            try:
                user_id = request.user.id
